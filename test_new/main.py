...
FLAGS = flags.FLAGS
flags.DEFINE_integer("steps_per_epoch", 300, "How many steps to do while one epoch iteration.")
...
flags.DEFINE_float("policy_learning_rate", 7e-6, "Learning rate for the policy function.")
flags.DEFINE_float("value_function_learning_rate", 1e-5, "Learning rate for the value funtion.")
flags.DEFINE_integer("train_policy_iterations", 75, "Max training iterations for policy funtion.")
flags.DEFINE_integer("train_value_iterations", 75, "Training iterations for value funtion.")
flags.DEFINE_float("lam", 0.9, "Smoothing parameter used for reducing variance.")
flags.DEFINE_float("target_kl", 0.01, "Target Kullback Leibler divergence to stop at if reached while policy update.")
...
buffer = Buffer((utils.screen_channel(), FLAGS.screen_resolution, FLAGS.screen_resolution), len(actions.FUNCTIONS), FLAGS.steps_per_epoch, lam=FLAGS.lam)

actor = PPO_Network_actor(FLAGS)
critic = PPO_Network_critic(FLAGS)


policy_optimizer = keras.optimizers.Adam(learning_rate=FLAGS.policy_learning_rate)
value_optimizer = keras.optimizers.Adam(learning_rate=FLAGS.value_function_learning_rate)
...
def step(screen, available_actions, available_actions_one_hot):    
    non_spatial_logits, spatial_out_logits = actor(screen, available_actions_one_hot)
    ...
    non_spatial_action = available_actions[tf.random.categorical([non_spatial_logits[available_actions]], 1)[0][0]]
...
for epoch in range(FLAGS.epochs):
    ...
    for t in range(FLAGS.steps_per_epoch):
        ...
        available_actions_one_hot = np.zeros([1, len(actions.FUNCTIONS)], dtype=np.float32)
        available_actions_one_hot[0, ] = 1

        func_actions, non_spatial_logits, spatial_out_logits, non_spatial_action, spatial_action, value_t = step(screen, available_actions, available_actions_one_hot)
        ...
        buffer.store(screen, available_actions_one_hot, non_spatial_action, spatial_action, observation_new[0].reward, value_t, (non_spatial_logprobability_t, spatial_logprobability_t))
        ...
        if terminal or (t == FLAGS.steps_per_epoch - 1):
            last_value = 0 if terminal else critic(screen, available_actions_one_hot)
